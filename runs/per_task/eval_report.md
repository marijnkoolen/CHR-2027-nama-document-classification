...
- All models were trained and evaluated on --device cpu throughout — MPS was avoided for its known correctness issues (silent NaNs, collapsed training) in the PyTorch versions this project uses.
- The four vision finetune checkpoints (vgg16-ft, efficientnet-ft, facebook__dinov2-small-ft, microsoft__dit-large-finetuned-rvlcdip-ft) needed a separate evaluation pass, run from a terminal with full filesystem access — the automated evaluation environment used for everything else hit an OS-level permission restriction reading raw page images directly (cache-based models were unaffected).
- None of the top-3 models per task use the finetune-image regime, so the pipeline combinations above aren't affected by that gap.
- Late fusion (averaging two independently fine-tuned models' softmax outputs, no training or architecture of its own) scores competitively on the standalone doc_type leaderboard — late-fusion-efficientnet_b0+bert-base-uncased comes within 0.0004 macro F1 of the standalone leader — and, once given genuine re-inference support (reloading its two constituent finetune checkpoints and running them on whatever page subset a pipeline run needs, rather than reusing their original test-set predictions), it turns out to win outright: knn-dinov2+mpnet → late-fusion-efficientnet_b0+bert-base-uncased is the best pipeline combination overall (LAS 0.9527), ahead of the previous best (LAS 0.9497) paired with the standalone-leaderboard's #4 model, bert-ft-bert-base-uncased. The same start_page-noise effect that let a modest standalone doc_type model win the original pipeline sweep applies here too.
